...
const NOTIFIABLE_STATUSES = ["CONFIRMED","PREPARING","READY","COMPLETED","CANCELLED"];
...
async function findCustomer(email){const q=`query ($businessId: ID!, $email: String) { business(id:$businessId) { customers(page:1,pageSize:10,email:$email) { edges { node { id name email mobile phone } } } } }`;const data=await wave(q,{businessId:process.env.WAVE_BUSINESS_ID,email});return data.business?.customers?.edges?.[0]?.node||null}
async function updateCustomer(customer,details){const q=`mutation ($input: CustomerPatchInput!) { customerPatch(input:$input) { didSucceed inputErrors { message code path } customer { id name email mobile phone } } }`;const data=await wave(q,{input:{id:customer.id,name:details.name,email:details.email,mobile:details.phone||null}});const out=data.customerPatch;if(!out.didSucceed)throw new Error(out.inputErrors?.map(e=>e.message).join("; ")||"Could not update customer in Wave.");return out.customer}
async function createCustomer(customer){const q=`mutation ($input: CustomerCreateInput!) { customerCreate(input:$input) { didSucceed inputErrors { message code path } customer { id name email mobile phone } } }`;const data=await wave(q,{input:{businessId:process.env.WAVE_BUSINESS_ID,name:customer.name,email:customer.email,mobile:customer.phone||null}});const out=data.customerCreate;if(!out.didSucceed)throw new Error(out.inputErrors?.map(e=>e.message).join("; ")||"Could not create customer in Wave.");return out.customer}
